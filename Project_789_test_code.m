...
T=u_opt(end);
v0=18.6182; %%%%%%%%%% Initial Speed %%%%%%%%%%%%%
x0=0;
v(1)=v0+(1/m)*(-(Crr*m*g)-(0.5*rho*(v0^2)*Cd*Aref))*T+u_opt(1)*T;
% v(1)=v0+u_opt(1)*T;
x(1)=x0+v0*T;
    for i=2:N
%         v(i)=v(i-1)+u_opt(i)*T;
        v(i)=v(i-1)+(1/m)*(-(Crr*m*g)-(0.5*rho*v(i-1)^2*Cd*Aref))*T+u_opt(i)*T;
        x(i)=x(i-1)+v(i-1)*T;
    end
%% Generating plots  
t=T:T:N*T
stairs(t,m*u_opt(1:N))
...
v0=18.6182;
% v(1)=v0+u(1)*T;
v(1)=v0+(1/m)*(-(Crr*m*g)-(0.5*rho*(v0^2)*Cd*Aref))*T+u(1)*T;
    for i=2:N
%         v(i)=v(i-1)+u(i)*T;
        v(i)=v(i-1)+(1/m)*(-(Crr*m*g)-(0.5*rho*v(i-1)^2*Cd*Aref))*T+u(i)*T;
%         x(i)=x(i-1)+v(i-1)*T;
    end
% J=sum(sum(u.*v))*T;
% J=(v*u)*T;
...
umin=0; %% Minimum value of acceleration
umax=2.5;  %% Maximum value of acceleration
x(1,1)=x0(1)+(1/m)*(-(Crr*m*g)-(0.5*rho*(x0(1)^2)*Cd*Aref))*T+u(1)*T;
% x(2,1)=x0(2)+x0(1)*T;
% x(1,1)=x0(1)+u(1)*T;
x(2,1)=x0(2)+x0(1)*T;
    for i=2:N
...
%         x(2,i)=x(2,i-1)+x(1,i-1)*T;
        ...
        x(1,i)=x(1,i-1)+(1/m)*(-(Crr*m*g)-(0.5*rho*x(1,i-1)^2*Cd*Aref))*T+u(i)*T;
        x(2,i)=x(2,i-1)+x(1,i-1)*T;
    end
for i=1:N
    g(i)=-x(1,i)+vmin;
