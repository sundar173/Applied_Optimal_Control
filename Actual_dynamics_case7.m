...
options=optimoptions('fmincon','Display','iter','Algorithm','sqp','maxiterations',1000,'MaxFunctionEvaluations',300000,'OptimalityTolerance',0.0000001);
...
if f==1
    ...
    y1=2203*ones(size(taxis_G));
    y2=0*ones(size(taxis_R));
    n=3;   %%%%%% no of traffic cycles to plot
    for i=1:n
           plot(taxis_G(i,:),y1,'g','LineWidth',2)
           hold on 
           vert1=taxis_G(i,end);
...
if (time>30 && time<60)  
       signal=60-time;
%     signal=100000;
 elseif (time>90 && time<120) %||(time>160 && time<180)
      signal=120-time;
%     signal=100000;
else
    signal=0;
